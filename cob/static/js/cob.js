...
$('#wNeighborsButton,#woNeighborsButton').click(function (evt) {
  ...
  if (hasNeighbors && (getOpt('visNeighbors') < 1)) {setOpt('visNeighbors');} else if (!(hasNeighbors)) {setOpt('visNeighbors', 0);}
...
function syncHPO(check, target) {
  ...
  if (check !== $('#' + target).hasClass('active')) {$('#' + target).button('toggle');}

  // Turn the rest of the window size & flank limit fields on or off
  if (!check) {$('.hpo-toggle').removeAttr('disabled');} else {$('.hpo-toggle').attr('disabled', 'disabled');}
}

// Observers to handle when the HPO buttons change
new MutationObserver(function (ms) {
  ms.forEach(function (m) {syncHPO($(m.target).hasClass('active'), 'hpo1');});
}).observe(document.getElementById('hpo'), obvConfig);
new MutationObserver(function (ms) {
  ms.forEach(function (m) {syncHPO($(m.target).hasClass('active'), 'hpo');});
